...
CREATE TABLE IF NOT EXISTS sales (
    ...
    rating FLOAT(2, 1),
    time_of_day VARCHAR(20) GENERATED ALWAYS AS (
        CASE
...
SELECT product_line,
    CASE
        WHEN AVG(quantity) > (SELECT avg_qnty FROM avg_quantity) THEN 'Good'
        ELSE 'Bad'
    END AS remark
FROM sales
GROUP BY product_line;
...
FROM sales
GROUP BY branch
HAVING SUM(quantity) > (SELECT avg_qnty FROM avg_quantity);
...
-- Best average ratings by day of the week per branch
...
-- Number of sales made by time of day on Sunday
